...
_load_bootstrap_env_main() {
  ...
  script_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" >/dev/null 2>&1 && pwd)" || {
    _load_bootstrap_env_error "Unable to resolve the workloads org_units script directory."
    return 1
  }
  ...
  if [[ "$landing_zone_drift_status" != "IN_SYNC" ]]; then
    _load_bootstrap_env_error \
      "Bootstrap output 'landing_zone_drift_status' is '${landing_zone_drift_status}', not 'IN_SYNC'. Resolve Control Tower drift before operating the workloads org_units root."
    return 1
  fi
  ...
  if [[ "$identity_center_enabled_baseline_status" != "SUCCEEDED" ]]; then
    _load_bootstrap_env_error \
      "The enabled IdentityCenterBaseline has status '${identity_center_enabled_baseline_status:-<empty>}', not 'SUCCEEDED'. Wait for or repair baseline enablement before operating the workloads org_units root."
    return 1
  fi
