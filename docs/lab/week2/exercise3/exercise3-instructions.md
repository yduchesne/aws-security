...
```bash
export TF_VAR_source_profile="week2-source"
export TF_VAR_external_id="aws-security-exercise3-<unique-customer-id>"
```

`external_id` is an identifier used to prevent confused-deputy attacks; it is
not a password. Do not use a password, access key, or other credential as its
value.
...
echo "role_arn: $(terraform -chdir=terraform/lab/week2/exercise3 output -raw role_arn)"
```
...
For Exercise 3, the central comparison is: **Require sts:externalid in a third-party trust policy.** Do
